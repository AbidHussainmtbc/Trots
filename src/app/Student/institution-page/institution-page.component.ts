...
export class InstitutionPageComponent implements OnInit {
  ...
  fnSaveInstituteInfo(): any {
    // this.router.navigateByUrl('std/Reg/StudentLocation');
    // return;
    ...
    if (this.checkValidation()) {
      // this.isLoading = true;
      // if()
      sessionStorage.setItem("institutename", this.instituteForm.value.orgName)
      this.router.navigateByUrl('std/Reg/StudentLocation');
      // this.apiService.Add_Std_EducationalInfo(this.institutionData).subscribe(rtnData => {
      //   this.isLoading = false;
      //   debugger;
